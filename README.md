...
> **v0.37.0** consolida la arquitectura progresiva actual. El frontend usa `/api/search/cache/` y `/api/search/store/`; `/api/search/` se conserva solo por compatibilidad y ahora recorre el mismo pipeline protegido. Mercadia en producción responde prioritariamente desde el catálogo persistente subido por el Bridge de Windows.
